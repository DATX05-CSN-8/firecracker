...
#[derive(Debug)]
pub enum TpmError { 
    TpmEmulatorError(String), // AAA TODO ? 
    TpmInitError(String),
}
...
const TPM_CRB_ADDR_BASE: u32 = TPM_START as u32; //TODO AAA
...
impl Tpm {
    pub fn new(path: String) -> Result<Self> {
        let emulator = Emulator::new(path)
            .map_err(|e| TpmError::TpmInitError(format!("{} {:?}","Failed while initializing tpm Emulator:", e)))?;
        let mut tpm = Tpm {
            emulator,
    ...
    fn reset(&mut self) -> Result<()> {
        ...
        if let Err(e) = self.emulator.startup_tpm(self.backend_buff_size) {
            return Err(TpmError::TpmInitError(format!("{} {:?}","Failed while running Startup TPM. Error:", e)));
        }
        Ok(())
    }
}
